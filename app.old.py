...
from wine_recommendation import recommend_wines
    ...
    with st.container():
        ...
        with cols[0]:
            ...
            if image_url:
                ...
                st.image("wine_placeholder.png", width=80)

        with cols[1]:
            st.markdown(f"<h3>🍷 {wine_data.get('name', 'Unknown')}</h3>", unsafe_allow_html=True)
            st.markdown(
                f"<div class='wine-subtext'>{wine_data.get('country_of_origin', 'N/A')} · {wine_data.get('category', 'N/A')}</div>",
                unsafe_allow_html=True
            )
            st.markdown(f"<div class='wine-meta'>**Price:** ${wine_data.get('price', 'N/A')}</div>", unsafe_allow_html=True)
            ...
            if rating_json_str:
                try:
                    rating_dict = json.loads(rating_json_str)
                    if isinstance(rating_dict, str):
                        rating = json.loads(rating_dict)  # double-decoded
                    else:
                        rating = rating_dict
                except json.JSONDecodeError:
                    rating = None

            rating_value = rating.get('ratingValue', 'N/A') if isinstance(rating, dict) else 'N/A'
            st.markdown(f"<div class='wine-meta'>**Rating:** {rating_value}</div>", unsafe_allow_html=True)

            rag_explanation = wine_data.get('rag_explanation')
            if rag_explanation:
                st.markdown(f"<div class='wine-meta'>**🤝 Why similar:** {rag_explanation}</div>", unsafe_allow_html=True)
            else:
                st.markdown("**🤝 Why similar:** N/A")
            ...
            try:
                ...
                with st.expander("📝 Tasting Notes"):
                    for review in reviews:
                        st.markdown(f"**{review.get('author', 'Anonymous')}** (Rating: {review.get('rating', 'N/A')}): {review.get('review', '')}")
            except json.JSONDecodeError:
                st.warning("Could not parse tasting notes.")
...
def main():
    ...
    st.write("Find wines similar to your favorites. Type a wine below to begin:")

    wine_names = load_wine_names()

    # Fast autocomplete
    user_input = st.text_input("Type a wine name:", "")
    suggestions = [w for w in wine_names if user_input.lower() in w.lower()]

    wine_name = None
    if suggestions:
        wine_name = st.selectbox("Select wine", suggestions)
    elif user_input:
        st.warning("No matches found yet. Try typing fewer letters.")

    if st.button("🍷 Find Similar Wines"):
        if not wine_name:
            st.error("Please select a wine first.")
            return
        ...
        display_wine_card(original_wine)

        # Recommendations
        st.header("✨ Recommended Wines")
        for rec in recommendations:
            wine_data = json.loads(rec) if isinstance(rec, str) else rec
