    ...
    with st.container():
        ...
        with col2:
            ...
            if len(description) > 150:  # Show a snippet on the card
                st.markdown(f"<small>{description[:150]}... [Read More](#wine-details-{wine_data.get('name').replace(' ', '-')})</small>", unsafe_allow_html=True)
            else:
                st.markdown(f"<small>{description}</small>", unsafe_allow_html=True)

            rag_explanation = wine_data.get('rag_explanation')
            if rag_explanation:
                st.markdown(f"<details><summary>🤔 Why similar</summary><small>{rag_explanation}</small></details>", unsafe_allow_html=True)
